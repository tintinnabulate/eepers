...
procedure Eepers is
   ...
   begin
      Ada.Text_IO.Create (F, Ada.Text_IO.Out_File, File_Name);
      for C in Palette loop
         Ada.Text_IO.Put (F, C'Image);
         for Comp in HSV_Comp loop
            Ada.Text_IO.Put (F, Palette_HSV (C) (Comp)'Image);
         end loop;
         ...
         declare
            ...
            Line : Ada.Strings.Unbounded.Unbounded_String :=
              Ada.Strings.Unbounded.To_Unbounded_String
                (Ada.Text_IO.Get_Line (F));

            -- Description:
            --   Chops the given string into two parts, based onto the given pattern.
            function Chop_By
              (Src     : in Ada.Strings.Unbounded.Unbounded_String;
               Pattern : in String) return Chopped_String
            with Pre => Pattern /= ""
            is
               Space_Index : constant Natural :=
                 Ada.Strings.Unbounded.Index
                   (Source => Src, Pattern => Pattern);
               Result      : Chopped_String;
            begin
               if Space_Index = 0 then
                  Result :=
                    Chopped_String'
                      (Left  => Src,
                       Right => Ada.Strings.Unbounded.Null_Unbounded_String);
               else
                  Result :=
                    Chopped_String'
                      (Left  =>
                         Ada.Strings.Unbounded.Unbounded_Slice
                           (Src, 1, Space_Index - 1),
                       Right =>
                         Ada.Strings.Unbounded.Unbounded_Slice
                           (Src,
                            Space_Index + 1,
                            Ada.Strings.Unbounded.Length (Src)));
               end if;
            ...
            function Find_Color_By_Key
              (Key : in Ada.Strings.Unbounded.Unbounded_String) return Color
            is
               use type Ada.Strings.Unbounded.Unbounded_String;
            begin
               for C in Palette loop
                  if Key = C'Image then
                     return Color'(Exists => True, Pal => C);
                  end if;
               end loop;
               return Color'(Exists => False);
            end Find_Color_By_Key;

            Tmp : Chopped_String := Chop_By (Line, " ");
            Key : constant Ada.Strings.Unbounded.Unbounded_String := Tmp.Left;
            Co  : constant Color := Find_Color_By_Key (Key);

         begin
            Line := Tmp.Right;
            Line := Ada.Strings.Unbounded.Trim (Line, Ada.Strings.Left);
   ...
   exception
      when E : Ada.Text_IO.Name_Error =>
         Ada.Text_IO.Put_Line
           ("WARNING: could not load colors from file "
            & File_Name
            & ": "
            & Exception_Message (E));
   end;
   ...
   procedure Delete_Path_Map is new
     Ada.Unchecked_Deallocation (Path_Map, Path_Map_Access);
   ...
   type Tutorial_Phase is
     (Tutorial_Move,
      Tutorial_Place_Bombs,
      Tutorial_Waiting_For_Sprint,
      Tutorial_Sprint,
      Tutorial_Done);
   ...
   begin
      for X in Start.X .. Start.X + Size.X - 1 loop
         for Y in Start.Y .. Start.Y + Size.Y - 1 loop
            ...
            if Game.Map (Y, X) /= Cell_Floor
              and Game.Map (Y, X) /= Cell_Explosion
            then
               return False;
            end if;
   ...
   procedure Allocate_Eeper (Game : in out Game_State; Idx : out Eeper_Index)
   is
      Too_Many_Entities     : exception;
      Successful_Allocation : Boolean := False;
   begin
      ...
      if not Successful_Allocation then
   ...
   procedure Spawn_Gnome (Game : in out Game_State; Position : in IVector2) is
      Idx  : Eeper_Index;
   ...
   begin
      ...
      Game.Eepers (Idx) :=
        (Game.Eepers (Idx) with delta 
         ...
         Eyes_Target   => Position + (Size.X / 2, Size.Y),
   ...
   procedure Game_Player_Turn
     (Game : in out Game_State; Step_Direction : Direction) is
   begin
   ...
   procedure Game_Update_Camera (Game : in out Game_State) is
      ...
      Camera_Offset   : constant Vector2 :=
        Screen_Size * 0.5 - Cell_Size * 0.5;
   ...
   function Command_Dequeue
     (Q : in out Command_Queue_Record; C : out Command) return Boolean is
   begin
      if Q.Size = 0 then
         return False;
      end if;
                     ...
                     then
                        if Eeper.Attack_Cooldown <= 0 then
                           declare
                              ...
                              Available_Positions :
                                array (0 .. Direction_Vector'Length - 1)
                                of IVector2;
                              Count               : Integer := 0;
                           begin
                              for Dir in Direction loop
   ...
   begin
      for Eeper of Game.Eepers loop
         declare
            ...
            Size                   : constant Vector2 :=
              To_Vector2 (Eeper.Size) * Cell_Size;
            Eyes_Angular_Direction : constant Float :=
              Delta_Angle
                (Eeper.Eyes_Angle,
                 Look_At
